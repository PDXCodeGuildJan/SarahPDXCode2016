...
class Angry_D(): 
	...
	def rules(self):
		...
		player_rule_answer = input("\nWould you like to see the rules? Type" \
		" 'Y' for yes or anything else for no.")

		if player_rule_answer.upper() == "Y":
			print("\n~~~~~~~~~~~Here are the rules:~~~~~~~~~~~\n\n" \
				"------The Battle------\nPlayers roll their dice at the" \
				" **same time**, trying to get from 1 to 6 the fastest.\nThe" \
				"first to do so wins! \n\n------The Details------\nStage 1," \
				...
				" showing 5 pips, another showing 6 pips.\n\n\nPlayers do not" \
				" have to perfectly roll each Stage;\nif a die shows one face" \
				"in a set, that die is locked (left aside) and the player" \
				"now rolls the other die to complete the set.\n\nEXCEPTION:" \
				"The 6 die face may never be locked! \n\n" \
				"-------The Anger-------\nIf the dice ever show both Angry" \
				"Faces, the player must START OVER from **Stage 1**."\
				"\n\n-------The Victory-------\nThe first player to race" \
				"through all Stages to reach Stage 3 and announces" \
				"'GET ANGRY!' is declared the victor!")

		input("\n\n\nPress ENTER to start the game and to roll the dice! Your" \
			"first Stages' goal is: [1, 2]")
	...
	def player_choice(self):
		...
		while invalid_choice:
			...
			entree = input("\nWould you like to reroll (a)ll of the die, just" \
				"the (t)op die or just the (b)ottom die? Please enter" \
				"(e) to exit.")
			...
			#Lock/Hold die_2, roll die_1 
				...
				print("We are so sorry to see you go, I was looking forward" \
					" to you getting so ANGRY!!!")
				exit()
